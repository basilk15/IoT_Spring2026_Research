...
#error "This sketch requires FreeRTOS trace facility and run-time stats enabled in the ESP32 Arduino core."
#endif

const char *WIFI_SSID = "pop-os";
const char *WIFI_PASS = "111222333";
const char *SERVER_IP = "10.68.249.223";
...
static constexpr const char *kFLTaskName = "FLTask";
static constexpr UBaseType_t kFLTaskPriority = 2;
...
TaskHandle_t gFLTaskHandle = nullptr;
...
bool parseModelResponse(const String &payload, int &round_out, float *weights_out) {
  ...
  int idx = 0;
  int pos = 0;
  while (idx < TOTAL_WEIGHTS && pos < static_cast<int>(arr.length())) {
...
void sgd_step(const float *x, const float *target) {
  ...
  for (int j = 0; j < OUTPUT_DIM; j++) {
    b3[j] -= LEARNING_RATE * dout[j];
  }
  for (int i = 0; i < HIDDEN2; i++) {
    dh2[i] = (h2[i] > 0) ? dh2[i] : 0;
  }
  ...
  for (int j = 0; j < HIDDEN2; j++) {
    b2[j] -= LEARNING_RATE * dh2[j];
  }
  for (int i = 0; i < HIDDEN1; i++) {
    dh1[i] = (h1[i] > 0) ? dh1[i] : 0;
  }
...
void captureStats(StatsSnapshot &snapshot) {
  ...
  if (statusArray == nullptr) {
    Serial.println("[CPU] Failed to allocate task stats buffer");
    return;
  }
...
void printStatsDelta(const StatsSnapshot &before, const StatsSnapshot &after) {
  if (after.totalRunTime <= before.totalRunTime) {
    Serial.println("[CPU] Run-time counter did not advance");
    return;
  }

  configRUN_TIME_COUNTER_TYPE totalDelta = after.totalRunTime - before.totalRunTime;
  Serial.println("[CPU] Task usage during one FL round");
  Serial.println("[CPU] Task Name         Delta(us)   Share");

  for (UBaseType_t i = 0; i < after.count; ++i) {
    const TaskRuntimeSnapshot &task = after.tasks[i];
    ...
    Serial.printf("[CPU] %-16s %-10lu %.2f%%\n",
                  task.name,
                  static_cast<unsigned long>(delta),
                  share);
  }

  Serial.printf("[CPU] TOTAL_RUNTIME_DELTA(us): %lu\n", static_cast<unsigned long>(totalDelta));
  Serial.println();
}

void flTask(void *parameter) {
  TaskHandle_t controllerHandle = static_cast<TaskHandle_t>(parameter);
  float samples[LOCAL_SAMPLES][INPUT_DIM];

  for (;;) {
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

    Serial.println("\n[FL] Starting federated round");
    download_model();
    ...
    for (int epoch = 0; epoch < LOCAL_EPOCHS; epoch++) {
      ...
      Serial.printf("[FL] Epoch %d loss=%.4f\n", epoch + 1, loss / (collected - 1));
    }

    upload_weights(collected);
    Serial.println("[FL] Federated round complete");

    xTaskNotifyGive(controllerHandle);
  }
}
...
void setup() {
  ...
  if (ok != pdPASS) {
    Serial.println("[FL] Failed to create FLTask");
    while (true) {
      delay(1000);
    }
  }
